...
Util.checkLogin = (req, res, next) => {
  if (req.session && req.session.loggedin) { // ✅ check session
    next()
  } else {
...
* Only allow access to inventory admin routes
**************************************** */
Util.checkAdmin = async (req, res, next) => {
  const token = req.cookies.jwt;

  if (!token) {
    let nav = await Util.getNav(req);
    return res.status(401).render("account/login", {
      title: "Login",
      nav,
      errors: null,
      account_email: "",
      message: "You must log in as Employee or Admin to access this page",
      description: "Login required"
    });
  }

  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, async (err, accountData) => {
    if (err) {
      res.clearCookie("jwt");
      let nav = await Util.getNav(req);
      return res.status(403).render("account/login", {
        ...
        errors: null,
        account_email: "",
        message: "Invalid login. Please log in again",
        description: "Login required"
      });
    }
